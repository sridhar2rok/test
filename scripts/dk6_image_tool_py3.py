...
def get_symbol_value(file, symb_name):
    ...
    objdump = subprocess.check_output(['arm-none-eabi-objdump', '--syms', file]).decode('utf-8')

    symb_re = re.compile(r'^([0-9a-f]{8})[\s\S]+[\s]+([0-9a-f]{8})\s([\w\.]+)')

    for ln in io.StringIO(objdump):
        m = symb_re.match(ln)
        if m:
...
def parse_sections(file):
    ...
    objdump = subprocess.check_output(['arm-none-eabi-objdump', '-h', file]).decode('utf-8')
...
def print_sota_img_directory_cmd(blobNumber):
    print("=================> SOTA information")
    # Generate the image directory command that will be used to provison the device
    valueToPrint = ".\\JN518xProgrammer.exe -V5 -s <COM_PORT> -P 1000000 -w PSECT:64@0x160="
    for i in range(1,9):
        bootable = "00"
        if i==1:
            bootable = "01"
        ...
        if position_in_flash != "00":
            print("Position in flash = "+ position_in_flash+" - targetAddr = 0x" +blobTargetAddr)
    ...
    print("=====> Image directory command")
    print(valueToPrint)
    print("=================> (end) SOTA information")
...
if JN518x_ES1 == 1: # deprecated
    ...
    dump_section = subprocess.check_output(['arm-none-eabi-objcopy', '--dump-section', '%s=data.bin' % last_section.name, args.in_file]).decode('utf-8')
    ...
    update_section = subprocess.check_output(['arm-none-eabi-objcopy', '--update-section', '%s=data.bin' % last_section.name, args.in_file, args.out_file]).decode('utf-8')
...
######################
    ...
    bin_output = subprocess.check_output(['arm-none-eabi-objcopy', '-O', 'binary', elf_file_name, bin_file_name]).decode('utf-8')
   ...
   #Default value initialization
    ...
    with open('data.bin', 'ab') as out_file:
        out_file.write(compatibility.encode('utf-8'))
        out_file.write(padding_bytes)
        out_file.write(boot_block)
        out_file.write(certificate.encode('utf-8'))
