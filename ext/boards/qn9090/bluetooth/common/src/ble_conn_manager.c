...
bool_t mbControllerPrivacyEnabled = FALSE;
...
{
    BLECONN_DBG_LOG("type=0x%x", pGenericEvent->eventType);
    ...
    (defined(gAppUseBonding_d) && (gAppUseBonding_d == 1U))
        ...
        case gBondCreatedEvent_c:
        {
            ...
            gcBondedDevices++;
            BLECONN_DBG_LOG("gcBondedDevices++");
        }
        break;
    ...
    (defined(gAppUseBonding_d) && (gAppUseBonding_d == 1U))
        ...
        {
            BLECONN_DBG_LOG("newControllerPrivacyState = %d", pGenericEvent->eventData.newControllerPrivacyState);
        ...
        {
            BLECONN_DBG_LOG("newHostPrivacyState = %d", pGenericEvent->eventData.newHostPrivacyState);
        }
        break;
#endif /* gAppUseBonding_d && gAppUsePrivacy_d */
        ...
        {
            BLECONN_DBG_LOG("Internal Error code=0x%x, errorSource=0x%x", pGenericEvent->eventData.internalError.errorCode, pGenericEvent->eventData.internalError.errorSource);
          /* Do not assert on this condition : this happens on a Disconnect event */
#ifdef DEBUG
          PRINTF("Internal Error code=%x\r\n", pGenericEvent->eventData.internalError.errorCode);
#endif
        }
        break;
...
{
    BLECONN_DBG_LOG("Event=%x", pConnectionEvent->eventType);
...
{
    BLECONN_DBG_LOG("EvtType=%x", pConnectionEvent->eventType);
    switch (pConnectionEvent->eventType)
    {
        case gConnEvtConnected_c:
        {
...
void BleConnManager_GapCommonConfig(void)
{
    ...
    {
        BLECONN_DBG_LOG("Adding %d devices to WL", gcBondedDevices);
...
STATIC bleResult_t BleConnManager_ManagePrivacyInternal(bool_t bCheckNewBond)
{
    ...
    BLECONN_DBG_LOG("checkNewBond=%d", bCheckNewBond);
    ...
    if( NULL != pOutIdentityAddresses )
    {
        ...
        {
            BLECONN_DBG_LOG("identitiesCount=%d mcDevicesInResolvingList=%d", identitiesCount, mcDevicesInResolvingList);
            if ((identitiesCount == (mcDevicesInResolvingList + 1U)) || (bCheckNewBond == FALSE))
            {
                ...
                if (gBleSuccess_c == result)
                    mbControllerPrivacyEnabled = TRUE;
        ...
        {
            BLECONN_DBG_LOG("Enable Host Privacy Irk", identitiesCount, mcDevicesInResolvingList);
            result = Gap_EnableHostPrivacy(TRUE, gSmpKeys.aIrk);
        }
