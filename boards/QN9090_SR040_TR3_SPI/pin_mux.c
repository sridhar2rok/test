...
void BOARD_InitSwoPins(void)
{
    if (IS_DEBUG_SESSION) {
        const uint32_t swoCfg = (/* Pin is configured as SWO */
            SWO_FUN |
            ...
            /* SSEL is disabled */
            IOCON_PIO_SSEL_DI);
        /* PORT0 PIN14  is configured as SWO */
        IOCON_PinMuxSet(IOCON, SWO_PORT, SWO_PIN, swoCfg);
    }

    const uint32_t uartTxCfg = (UWB_UART_TX_FUN |
                                /* Selects pull-up function */
                                IOCON_PIO_MODE_PULLUP |
                                /* Standard mode, output slew rate control is disabled */
                                IOCON_PIO_SLEW0_STANDARD |
    ...
    const uint32_t uartRxCfg = (UWB_UART_RX_FUN |
                                /* Selects pull-up function */
                                IOCON_PIO_MODE_PULLUP |
                                /* Standard mode, output slew rate control is disabled */
                                IOCON_PIO_SLEW0_STANDARD |
    ...
    IOCON_PinMuxSet(IOCON, UWB_UART_TX_PORT, UWB_UART_TX_PIN, uartTxCfg);
    IOCON_PinMuxSet(IOCON, UWB_UART_RX_PORT, UWB_UART_RX_PIN, uartRxCfg);
}

void BOARD_InitHeliosPins(void)
{
    const uint32_t sckCfg = (UWB_HELIOS_CLK_FUN |
                             /* IOCON_PIO_MODE_PULLUP |*/
                             /* Standard mode, output slew rate control is disabled */
                             IOCON_PIO_SLEW0_STANDARD |
                             /* Input function is not inverted */
    ...
    const uint32_t misoCfg = (UWB_HELIOS_MISO_FUN |
                              /* Selects pull-up function */
                              IOCON_PIO_MODE_PULLUP |
                              /* Standard mode, output slew rate control is disabled */
                              IOCON_PIO_SLEW0_STANDARD |
    ...
    const uint32_t mosiCfg = (UWB_HELIOS_MOSI_FUN |
                              /* Selects pull-up function */
                              IOCON_PIO_MODE_PULLUP |
                              /* Standard mode, output slew rate control is disabled */
                              IOCON_PIO_SLEW0_STANDARD |
    ...
    const uint32_t csCfg = (UWB_HELIOS_CS_FUN |
                            /* Selects pull-up function */
    ...
    const uint32_t accelCsCfg = (UWB_ACCEL_CS_FUN |
    ...
    gpio_pin_config_t paGpioCfg = {.pinDirection = kGPIO_DigitalOutput, .outputLogic = 0U};

    gpio_pin_config_t vibAlertGpioCfg = {.pinDirection = kGPIO_DigitalOutput, .outputLogic = 0U};

    gpio_pin_config_t csGpioCfg  = {.pinDirection = kGPIO_DigitalOutput, .outputLogic = 1U};
    gpio_pin_config_t rstGpioCfg = {.pinDirection = kGPIO_DigitalOutput, .outputLogic = 1U};

    const uint32_t paCfg = (UWB_PA_ENABLE_FUN |
                            /* Selects no pull-down/pull-up function */
    ...
    const uint32_t vibAlertCfg = (UWB_VIB_ALERT_FUN |
                                  /* Selects no pull-down/pull-up function */
                                  IOCON_PIO_MODE_INACT |
                                  /* Standard mode, output slew rate control is disabled */
                                  IOCON_PIO_SLEW0_STANDARD |
    ...
    const uint32_t rstCfg = (UWB_RST_N_FUN |
                             /* Selects no pull-down/pull-up function */
                             IOCON_PIO_MODE_INACT |
                             /* Standard mode, output slew rate control is disabled */
                             IOCON_PIO_SLEW0_STANDARD |
    ...
    const uint32_t rdyCfg = (UWB_RST_N_FUN |
                             /* Selects pull-up function */
                             IOCON_PIO_MODE_INACT |
                             /* Standard mode, output slew rate control is disabled */
                             IOCON_PIO_SLEW0_STANDARD |
    ...
    const uint32_t irqCfg = (UWB_HELIOS_IRQ_FUN |
                             /* Selects pull-up function */
                             IOCON_PIO_MODE_INACT |
                             /* Standard mode, output slew rate control is disabled */
                             IOCON_PIO_SLEW0_STANDARD |
    ...
    IOCON_PinMuxSet(IOCON, UWB_HELIOS_MOSI_PORT, UWB_HELIOS_MISO_PIN, misoCfg);
    IOCON_PinMuxSet(IOCON, UWB_HELIOS_MOSI_PORT, UWB_HELIOS_MOSI_PIN, mosiCfg);
    IOCON_PinMuxSet(IOCON, UWB_HELIOS_CS_PORT, UWB_HELIOS_CS_PIN, csCfg);
    IOCON_PinMuxSet(IOCON, UWB_HELIOS_CLK_PORT, UWB_HELIOS_CLK_PIN, sckCfg);
    IOCON_PinMuxSet(IOCON, UWB_HELIOS_IRQ_PORT, UWB_HELIOS_IRQ_PIN, irqCfg);

    IOCON_PinMuxSet(IOCON, UWB_ACCEL_CS_PORT, UWB_ACCEL_CS_PIN, accelCsCfg);

    GPIO_PinInit(UWB_PA_ENABLE_GPIO, UWB_PA_ENABLE_PORT, UWB_PA_ENABLE_PIN, &paGpioCfg);
    GPIO_PinInit(UWB_VIB_ALERT_GPIO, UWB_VIB_ALERT_PORT, UWB_VIB_ALERT_PIN, &vibAlertGpioCfg);
    GPIO_PinInit(UWB_HELIOS_CS_GPIO, UWB_HELIOS_CS_PORT, UWB_HELIOS_CS_PIN, &csGpioCfg);
    GPIO_PinInit(GPIO, UWB_RST_N_PORT, UWB_RST_N_PIN, &rstGpioCfg);

    IOCON_PinMuxSet(IOCON, UWB_PA_ENABLE_PORT, UWB_PA_ENABLE_PIN, paCfg);
    IOCON_PinMuxSet(IOCON, UWB_VIB_ALERT_PORT, UWB_VIB_ALERT_PIN, vibAlertCfg);

    IOCON_PinMuxSet(IOCON, UWB_RST_N_PORT, UWB_RST_N_PIN, rstCfg);
    IOCON_PinMuxSet(IOCON, UWB_RDY_N_PORT, UWB_RDY_N_PIN, rdyCfg);

    GPIO_PinWrite(GPIO, UWB_HELIOS_CS_PORT, UWB_HELIOS_CS_PIN, 1);
    GPIO_PinWrite(GPIO, UWB_RST_N_PORT, UWB_RST_N_PIN, 1);
}

void BOARD_InitBuzzerPins()
{
    const uint32_t buzzerCfg = (BUZZER_FUN |
                                /* Selects pull-up function */
                                IOCON_PIO_MODE_PULLUP |
                                /* Standard mode, output slew rate control is disabled */
                                IOCON_PIO_SLEW0_STANDARD |
    ...
    IOCON_PinMuxSet(IOCON, BUZZER_PORT, BUZZER_PIN, buzzerCfg);
}
...
void BOARD_DeInitDebugUARTPins(void)
{
    ...
    gpio_pin_config_t DBG_UART_TX_config = {.pinDirection = kGPIO_DigitalInput, .outputLogic = 0U};
    /* Initialize GPIO functionality on pin PIO0_10 (pin 13)  */
    GPIO_PinInit(BOARD_DEINITDEBUGUARTPINS_DBG_UART_TX_GPIO,
        BOARD_DEINITDEBUGUARTPINS_DBG_UART_TX_PORT,
        BOARD_DEINITDEBUGUARTPINS_DBG_UART_TX_PIN,
        &DBG_UART_TX_config);

    gpio_pin_config_t DBG_UART_RX_config = {.pinDirection = kGPIO_DigitalInput, .outputLogic = 0U};
    /* Initialize GPIO functionality on pin PIO0_11 (pin 14)  */
    GPIO_PinInit(BOARD_DEINITDEBUGUARTPINS_DBG_UART_RX_GPIO,
    ...
    IOCON_PinMuxSet(
        IOCON, BOARD_DEINITDEBUGUARTPINS_DBG_UART_TX_PORT, BOARD_DEINITDEBUGUARTPINS_DBG_UART_TX_PIN, DBG_UART_TX);
    ...
    IOCON_PinMuxSet(
        IOCON, BOARD_DEINITDEBUGUARTPINS_DBG_UART_RX_PORT, BOARD_DEINITDEBUGUARTPINS_DBG_UART_RX_PIN, DBG_UART_RX);
}
...
void BOARD_DeInitAppUARTPins(void)
{
    ...
    gpio_pin_config_t APP_UART_TX_config = {.pinDirection = kGPIO_DigitalInput, .outputLogic = 0U};
    ...
    gpio_pin_config_t APP_UART_RX_config = {.pinDirection = kGPIO_DigitalInput, .outputLogic = 0U};
    ...
    IOCON_PinMuxSet(
        IOCON, BOARD_DEINITAPPUARTPINS_APP_UART_TX_PORT, BOARD_DEINITAPPUARTPINS_APP_UART_TX_PIN, APP_UART_TX);
    ...
    IOCON_PinMuxSet(
        IOCON, BOARD_DEINITAPPUARTPINS_APP_UART_RX_PORT, BOARD_DEINITAPPUARTPINS_APP_UART_RX_PIN, APP_UART_RX);
}
