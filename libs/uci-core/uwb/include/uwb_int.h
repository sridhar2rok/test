...
extern void uwb_ucif_app_data_transceive_rsp_status(tUWB_RESPONSE_EVT event, uint8_t* p_buf, uint16_t len);
...
extern void uwb_ucif_proc_app_data_send_ntf_status(uint8_t* p_buf, uint16_t len);
extern void uwb_ucif_proc_app_data_rcve_ntf_status(uint8_t* p_buf, uint16_t len);

/* APIs for handling UWB RF test command responses and notifications */
extern void uwb_ucif_test_management_status(tUWB_TEST_RESPONSE_EVT event, uint8_t* p_buf, uint16_t len);
...
extern void uwb_ucif_proc_rf_test_data(tUWB_TEST_RESPONSE_EVT event, uint8_t* p_buf, uint16_t len);
