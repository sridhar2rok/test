...
**
*******************************************************************************/
void uwa_sys_timer_update(uint16_t event) {
  UWB_HDR *p_msg;
    if ((p_msg = (UWB_HDR *) phUwb_GKI_getbuf (sizeof (UWB_HDR))) != NULL)
    {
        p_msg->event = event;
...
void uwa_sys_deregister(uint8_t id) {
  UCI_TRACE_D("uwa_sys: deregistering subsystem %i", id);
...
void uwa_sys_enable_subsystems(void) {
  ...
  UCI_TRACE_D("uwa_sys: enabling subsystems");
