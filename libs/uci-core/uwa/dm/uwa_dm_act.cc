...
static void uwa_dm_uwb_response_cback(tUWB_RESPONSE_EVT event,
  ...
  UCI_TRACE_D("uwa_dm_uwb_response_cback:%s(0x%x)", uwa_dm_uwb_revt_2_str(event), event);
  ...
  switch (event) {
    ...
    case UWB_DISABLE_REVT: /* 1  Disable event */
        dm_cback_data.status  = UWB_STATUS_OK;
        uwa_sys_deregister(UWA_ID_DM);
        /* Notify app */
        uwa_dm_cb.flags &= (uint32_t)(~UWA_DM_FLAGS_DM_IS_ACTIVE);
        (*uwa_dm_cb.p_dm_cback)(UWA_DM_DISABLE_EVT, &dm_cback_data);
        break;
    ...
    case UWB_GET_DEVICE_INFO_REVT:
      dm_cback_data.sGet_device_info.status = p_data->sGet_device_info.status;
      ...
      if (p_data->sGet_device_info.status == UWB_STATUS_OK) {
        ...
        dm_cback_data.sGet_device_info.device_info_len = p_data->sGet_device_info.device_info_len;
        memcpy(dm_cback_data.sGet_device_info.device_info, p_data->sGet_device_info.device_info,
            p_data->sGet_device_info.device_info_len);
      }
    ...
    case UWB_SESSION_GET_COUNT_REVT:
      dm_cback_data.sGet_session_cnt.status = p_data->sGet_session_cnt.status;

      if(p_data->sGet_session_cnt.status == UWB_STATUS_OK) {
        dm_cback_data.sGet_session_cnt.count = p_data->sGet_session_cnt.count;
      } else {
    ...
    case UWB_SESSION_GET_STATE_REVT:
      dm_cback_data.sGet_session_state.status = p_data->sGet_session_state.status;

      if(p_data->sGet_session_state.status == UWB_STATUS_OK) {
        dm_cback_data.sGet_session_state.session_state = p_data->sGet_session_state.session_state;
      } else {
    ...
    case UWB_RANGE_INTERVAL_UPDATE_REQ_REVT:
      if(p_data->status != UWB_STATUS_OK) {
        UCI_TRACE_E(" Range interval update request is failed");
      }
      dm_cback_data.status = p_data->status;
      (*uwa_dm_cb.p_dm_cback)(UWA_DM_RANGE_INTERVAL_UPDATE_REQ_RSP_EVT, &dm_cback_data);
      break;
    ...
    case UWB_APP_DATA_SEND_REVT:
      dm_cback_data.sApp_data_tx.status = p_data->sApp_data_tx.status;
      if (p_data->sApp_data_tx.status != UWB_STATUS_OK) {
          UCI_TRACE_E("app data send is failed");
      }

      (*uwa_dm_cb.p_dm_cback)(UWA_DM_APP_DATA_SEND_RSP_EVT, &dm_cback_data);
      break;

    case UWB_APP_DATA_RCVE_REVT:
      dm_cback_data.sApp_data_rx.status = p_data->sApp_data_rx.status;
      if (p_data->sApp_data_rx.status != UWB_STATUS_OK) {
          UCI_TRACE_E("app data receive is failed");
      }

      (*uwa_dm_cb.p_dm_cback)(UWA_DM_APP_DATA_RCVE_RSP_EVT, &dm_cback_data);
      break;

    case UWB_APP_DATA_SEND_STATUS_REVT:
      {
        tUWA_APP_DATA_SEND_STATUS_REVT* p_app_data = &dm_cback_data.sApp_data_tx_status;
        p_app_data->session_id = p_data->sApp_data_tx_status.session_id;
        p_app_data->status = p_data->sApp_data_tx_status.status;
      }
        (*uwa_dm_cb.p_dm_cback)(UWA_DM_APP_DATA_SEND_STATUS_NTF_EVT, &dm_cback_data);
      break;
    case UWB_APP_DATA_RCVE_STATUS_REVT:
      {
        tUWA_APP_DATA_RCVE_STATUS_REVT* p_app_data = &dm_cback_data.sApp_data_rx_status;
        p_app_data->session_id = p_data->sApp_data_rx_status.session_id;
        p_app_data->status = p_data->sApp_data_rx_status.status;
        p_app_data->data_len = p_data->sApp_data_rx_status.data_len;
        if(p_app_data->data_len > 0){
          memcpy(p_app_data->data, p_data->sApp_data_rx_status.data, p_data->sApp_data_rx_status.data_len);
        }
      }
        (*uwa_dm_cb.p_dm_cback)(UWA_DM_APP_DATA_RCVE_STATUS_NTF_EVT, &dm_cback_data);
      break;
...
**
*******************************************************************************/
static void uwa_dm_uwb_test_response_cback(tUWB_TEST_RESPONSE_EVT event, tUWB_TEST_RESPONSE *p_data)
{
    tUWA_DM_TEST_CBACK_DATA dm_cback_data;

    UCI_TRACE_D("uwa_dm_uwb_test_response_cback:%s(0x%x)", uwa_test_dm_uwb_revt_2_str(event), event);
    ...
    case UWB_TEST_LOOPBACK_DATA_REVT: {
        ...
        if (p_data->sRf_test_result.length > 0) {
            ...
            memcpy(
                &p_uwb_loopback_test_data->data[0], &p_data->sRf_test_result.data[0], p_uwb_loopback_test_data->length);
        }
...
bool uwa_dm_act_get_device_info(tUWA_DM_MSG* p_data){
  ...
  if(p_data == NULL) {
    ...
    if(UWB_STATUS_OK == status){
      UCI_TRACE_D("uwa_dm_act_get_device_info(): success ,status=0x%X",status);
    }else{
...
bool uwa_dm_set_core_config(tUWA_DM_MSG* p_data) {
  ...
  if (p_data->setconfig.length + 2 > 255) {
    ...
    status = UWB_SetCoreConfig(p_data->setconfig.num_ids,
        p_data->setconfig.length, p_data->setconfig.p_data);
  }
  if (status != UWB_STATUS_OK) {
    UCI_TRACE_E("uwa_dm_set_core_config(): failed ,status=0x%X",status);
  }
  else
  {
    UCI_TRACE_D("uwa_dm_set_core_config(): success ,status=0x%X",status);
  }
  return (true);
}
...
bool uwa_dm_get_core_config(tUWA_DM_MSG* p_data) {
  ...
  if(p_data == NULL) {
    UCI_TRACE_E("uwa_dm_get_core_config(): p_data is NULL)");
  } else {
    status = UWB_GetCoreConfig(p_data->getconfig.num_ids, p_data->getconfig.length, p_data->getconfig.p_pmids);

    if(UWB_STATUS_OK == status){
      UCI_TRACE_D("uwa_dm_get_core_config(): success ,status=0x%X",status);
    } else {
      UCI_TRACE_E("uwa_dm_get_core_config(): failed ,status=0x%X",status);
    }
  }
  return (true);
}
...
bool uwa_dm_act_device_reset(tUWA_DM_MSG* pResetConfig) {
  ...
  if(pResetConfig == NULL) {
    ...
    if(UWB_STATUS_OK == status){
      UCI_TRACE_D("uwa_dm_act_device_reset(): success ,status=0x%X",status);
    } else {
...
bool uwa_dm_act_get_session_count(tUWA_DM_MSG* p_data){
  ...
  if(p_data == NULL) {
    ...
    if(UWB_STATUS_OK == status){
      UCI_TRACE_D("uwa_dm_act_get_session_count(): success ,status=0x%X",status);
    }else{
...
bool uwa_dm_act_app_set_config(tUWA_DM_MSG* p_data) {
  ...
  {
      UCI_TRACE_D("uwa_dm_act_app_set_config(): success ,status=0x%X",status);
  }
...
bool uwa_dm_act_app_get_config(tUWA_DM_MSG* p_data) {
  ...
  if(p_data == NULL) {
    ...
    if(UWB_STATUS_OK == status){
      UCI_TRACE_D("uwa_dm_act_app_get_config(): success ,status=0x%X",status);
    } else {
...
/*******************************************************************************
**
** Function         uwa_dm_act_send_range_interval_update_request
**
** Description      send ranging interval update request command to change the ranging interval
**                  during ongoing ranging session
**
** Returns          FALSE (message buffer is NOT freed by caller)
**
*******************************************************************************/
bool uwa_dm_act_send_range_interval_update_request(tUWA_DM_MSG* p_data){
  if(p_data == NULL) {
    UCI_TRACE_E("uwa_dm_act_send_range_interval_update_request(): p_data is NULL)");
  } else {
    UWB_SendRangeIntervalUpdateRequest(p_data->rangeIntervalUpdateReq.session_id, p_data->rangeIntervalUpdateReq.interval);
  }
  return true;
}
...
/*******************************************************************************
**
** Function         uwa_dm_act_send_app_data
**
** Description      send application data over UWB
**
** Returns          FALSE (message buffer is NOT freed by caller)
**
*******************************************************************************/
bool uwa_dm_act_send_app_data(tUWA_DM_MSG* p_data) {
  if(p_data == NULL) {
    UCI_TRACE_E("uwa_dm_act_send_app_data(): p_data is NULL)");
  } else {
    UWB_SendAppData(p_data->sApp_data_tx.session_id,p_data->sApp_data_tx.dest_addr, p_data->sApp_data_tx.data_len,
                  p_data->sApp_data_tx.app_data);
  }
  return true;
}
...
/*******************************************************************************
**
** Function         uwa_dm_act_rcve_app_data
**
** Description      send application data over UWB
**
** Returns          FALSE (message buffer is NOT freed by caller)
**
*******************************************************************************/
bool uwa_dm_act_rcve_app_data(tUWA_DM_MSG* p_data) {
  if(p_data == NULL) {
    UCI_TRACE_E("uwa_dm_act_rcve_app_data(): p_data is NULL)");
  } else {
    UWB_ReceiveAppData(p_data->sApp_data_rx.session_id,p_data->sApp_data_rx.src_addr);
  }
  return true;
}
...
bool uwa_dm_act_get_range_count(tUWA_DM_MSG* p_data) {
  ...
  if(p_data == NULL) {
    ...
    if(UWB_STATUS_OK == status){
      UCI_TRACE_D("uwa_dm_act_get_range_count(): success ,status=0x%X",status);
    } else {
...
bool uwa_dm_act_get_session_status(tUWA_DM_MSG* p_data) {
  ...
  if(p_data == NULL) {
    ...
    if(UWB_STATUS_OK == status){
      UCI_TRACE_D("uwa_dm_act_get_session_status(): success ,status=0x%X",status);
    } else {
...
bool uwa_dm_act_get_device_capability(tUWA_DM_MSG* p_data){
  ...
  if(p_data == NULL) {
    ...
    if(UWB_STATUS_OK == status){
      UCI_TRACE_D("uwa_dm_act_get_device_capability(): success ,status=0x%X",status);
    } else {
...
bool uwa_dm_act_multicast_list_update(tUWA_DM_MSG* p_data){
  ...
  if(p_data == NULL) {
    ...
    status = UWB_MulticastListUpdate(p_data->sMulticast_list.session_id, p_data->sMulticast_list.action, p_data->sMulticast_list.no_of_controlee, p_data->sMulticast_list.short_address_list, p_data->sMulticast_list.subsession_id_list);
    if(UWB_STATUS_OK == status){
      UCI_TRACE_D("uwa_dm_act_multicast_list_update(): success ,status=0x%X",status);
    } else {
...
bool uwa_dm_act_test_set_config(tUWA_DM_MSG* p_data) {
  ...
  {
      UCI_TRACE_D("uwa_dm_act_test_set_config(): success ,status=0x%X",status);
  }
...
bool uwa_dm_act_test_get_config(tUWA_DM_MSG* p_data) {
  ...
  if(p_data == NULL) {
    ...
    if(UWB_STATUS_OK == status){
      UCI_TRACE_D("uwa_dm_act_test_get_config(): success ,status=0x%X",status);
    } else {
...
bool uwa_dm_act_test_periodic_tx(tUWA_DM_MSG* p_data) {
  ...
  if(p_data == NULL) {
    ...
    if(UWB_STATUS_OK == status){
      UCI_TRACE_D("uwa_dm_act_test_periodic_tx(): success ,status=0x%X",status);
    } else {
...
bool uwa_dm_act_test_per_rx(tUWA_DM_MSG* p_data) {
  ...
  if(p_data == NULL) {
    ...
    if(UWB_STATUS_OK == status){
      UCI_TRACE_D("uwa_dm_act_test_per_rx(): success ,status=0x%X",status);
    } else {
...
bool uwa_dm_act_test_uwb_loopback(tUWA_DM_MSG* p_data) {
  ...
  if(p_data == NULL) {
    ...
    if(UWB_STATUS_OK == status){
      UCI_TRACE_D("uwa_dm_act_test_uwb_loopback(): success ,status=0x%X",status);
    } else {
...
bool uwa_dm_act_test_stop_session(tUWA_DM_MSG* p_data){
  ...
  if(p_data == NULL) {
    ...
    if(UWB_STATUS_OK == status){
      UCI_TRACE_D("uwa_dm_act_test_stop_session(): success , status=0x%X",status);
    }else{
...
bool uwa_dm_act_test_rx(tUWA_DM_MSG *p_data)
{
    ...
    if (p_data == NULL) {
        ...
        status = UWB_TestRx();
        if (UWB_STATUS_OK == status) {
            UCI_TRACE_D("uwa_dm_act_test_rx(): success , status=0x%X", status);
        }
        else {
...
**
*******************************************************************************/
const char * uwa_dm_uwb_revt_2_str(tUWB_RESPONSE_EVT event)

{
  switch (event) {
    ...
    case UWB_RANGE_INTERVAL_UPDATE_REQ_REVT:
      return "UWB_RANGE_INTERVAL_UPDATE_REQ_REVT";

    case UWB_APP_DATA_SEND_REVT:
      return "UWB_APP_DATA_SEND_REVT";

    case UWB_APP_DATA_RCVE_REVT:
      return "UWB_APP_DATA_RCVE_REVT";

    case UWB_APP_DATA_SEND_STATUS_REVT:
      return "UWB_APP_DATA_SEND_STATUS_REVT";

    case UWB_APP_DATA_RCVE_STATUS_REVT:
      return "UWB_APP_DATA_RCVE_STATUS_REVT";
...
**
*******************************************************************************/
const char *uwa_test_dm_uwb_revt_2_str(tUWB_TEST_RESPONSE_EVT event)
