...
/*session is ongoing */
#define UWA_STATUS_SESSIONS_ONGOING UCI_STATUS_SESSIONS_ONGOING
/*session multicast list is full */
#define UWA_STATUS_SESSIONS_MULTICAST_LIST_FULL UCI_STATUS_SESSIONS_MULTICAST_LIST_FULL
/*session address is not found */
#define UWA_STATUS_SESSIONS_ADDRESS_NOT_FOUND UCI_STATUS_SESSIONS_ADDRESS_NOT_FOUND
/*session address is already present */
#define UWA_STATUS_SESSIONS_ADDRESS_ALREADY_PRESENT UCI_STATUS_SESSIONS_ADDRESS_ALREADY_PRESENT
...
#define UWA_STATUS_DATA_MAX_TX_APDU_SIZE_EXCEEDED UCI_STATUS_DATA_MAX_TX_APDU_SIZE_EXCEEDED
...
typedef enum Response_Event
{
    ...
    UWA_DM_ENABLE_EVT = UWA_DM_EVENT,
    ...
    /* Result of UWB error ntf */
    UWA_DM_UWB_ERROR_EVT,
    ...
    /* Result of setAppConfig */
    UWA_DM_SESSION_SET_CONFIG_RSP_EVT,
    /* Result of getAppConfig */
    ...
    /* Result of range data ntf */
    UWA_DM_RANGE_DATA_NTF_EVT,
    /* Result of range interval update req cmd */
    UWA_DM_RANGE_INTERVAL_UPDATE_REQ_RSP_EVT,
    /* Results of app data send cmd */
    UWA_DM_APP_DATA_SEND_RSP_EVT,
    /*  Results of app data tx notification*/
    UWA_DM_APP_DATA_SEND_STATUS_NTF_EVT,
    /* Results of app data send cmd */
    UWA_DM_APP_DATA_RCVE_RSP_EVT,
    /*  Results of app data rx notification*/
    UWA_DM_APP_DATA_RCVE_STATUS_NTF_EVT,
    /* Result of range get count */
    UWA_DM_GET_RANGE_COUNT_RSP_EVT,
    /* Results of UWA_GetCoreDeviceCapability  */
...
typedef enum Response_Test_Event
{
    /* Result of test set config event */
    UWA_DM_TEST_SET_CONFIG_RSP_EVT = UWA_TEST_DM_EVENT,
...
typedef struct {
  ...
  uint8_t device_info[UCI_DEVICE_INFO_MAX_SIZE];
...
typedef struct {
  ...
  uint8_t pdoa[4];
  uint8_t pdoaIndex[4];
  uint8_t aoa_dest[4];   /* Ranging AOA Report */
  uint8_t slot_index;
...
/* the data type associated with UWB_APP_DATA_SEND_REVT */
typedef struct {
  uint8_t status;
}tUWA_APP_DATA_SEND_REVT;

/* the data type associated with UWB_APP_DATA_SEND_STATUS_REVT */
typedef struct {
  uint32_t session_id;
  uint8_t status;
}tUWA_APP_DATA_SEND_STATUS_REVT;

/* the data type associated with UWB_APP_DATA_SEND_REVT */
typedef struct {
  uint8_t status;
}tUWA_APP_DATA_RCVE_REVT;
...
}tUWA_APP_DATA_RCVE_STATUS_REVT;
...
typedef union {
  ...
  tUWA_APP_DATA_SEND_REVT sApp_data_tx;  /* UWA_DM_APP_DATA_SEND_RSP_EVT */
  tUWA_APP_DATA_RCVE_REVT sApp_data_rx;  /* UWA_DM_APP_DATA_RCVE_RSP_EVT */
  tUWA_APP_DATA_SEND_STATUS_REVT sApp_data_tx_status;  /* UWA_DM_APP_DATA_SEND_STATUS_NTF_EVT */
  tUWA_APP_DATA_RCVE_STATUS_REVT sApp_data_rx_status;  /* UWA_DM_APP_DATA_RCVE_STATUS_NTF_EVT */
  tUWA_GET_CORE_DEVICE_CAPABILITY sGet_device_capability; /*UWA_DM_GET_CORE_DEVICE_CAPABILITY_RSP_EVT*/
...
/* UWA_DM callback */
typedef void(tUWA_DM_CBACK)(eResponse_Event event, tUWA_DM_CBACK_DATA *p_data);
typedef void(tUWA_DM_TEST_CBACK)(eResponse_Test_Event event, tUWA_DM_TEST_CBACK_DATA *p_data);
...
**                  reported with an CORE_SET_CONFIG_RSP_EVT in the tUWA_DM_CBACK
**                  callback.
**
...
**
*******************************************************************************/
extern tUWA_STATUS UWA_SetCoreConfig(uint8_t noOfParams, uint8_t paramLen, tUWA_PMID* p_param_ids);
...
**
*******************************************************************************/
extern tUWA_STATUS UWA_GetCoreConfig(uint8_t noOfParams, uint8_t paramLen, tUWA_PMID* p_param_ids);
...
**
** Description      This function is called to send the ranging interval update request command to
**                  change the ranging interval during ongoing ranging session
**
...
**
**                  data_len  - application raw data length
**                  data_buff - application raw data
**
** Returns          UWA_STATUS_OK if successfully initiated
**                  UWA_STATUS_FAILED otherwise
**
...
**
**                  session id - value of particular session ID
**                  srcAddress - src address from where app data received
**
** Returns          UWA_STATUS_OK if successfully initiated
**                  UWA_STATUS_FAILED otherwise
**
